 ...
 * @since 2017/1/23
 */
public class GirlDetailAdapter2 extends BasePagerAdapter<GirlResult2.ImgsBean>{

    private LayoutInflater layoutInflater;

    public GirlDetailAdapter2(Context context, List<GirlResult2.ImgsBean> listData) {
        super(context,listData);
        this.layoutInflater = LayoutInflater.from(context);
    }

    @Override
    public View getView(ViewGroup container, final GirlResult2.ImgsBean girl, int position) {
        View view = layoutInflater.inflate(R.layout.list_girl_detail_item,container,false);
        final PhotoView photoView = (PhotoView)view.findViewById(R.id.photoView);
        photoView.enable();
        Glide.with(context).load(girl.getImageUrl()).diskCacheStrategy(DiskCacheStrategy.SOURCE).into(photoView);
                    ...
                    public void onItemClick(Object o, int position) {
                        switch (position){
                            ...
                            case 1:
                                ...
                                Glide.with(context).load(girl.getImageUrl()).asBitmap().into(new SimpleTarget<Bitmap>() {
                                    @Override
                                ...
                                break;
                            case 2:
                                UMImage image =  new UMImage(context,girl.getImageUrl());
                                new ShareAction((Activity) context)
                                        .withMedia(image)
                                        .setDisplayList(SHARE_MEDIA.SINA,SHARE_MEDIA.QQ,SHARE_MEDIA.QZONE,
                                                SHARE_MEDIA.WEIXIN,SHARE_MEDIA.WEIXIN_CIRCLE,SHARE_MEDIA.WEIXIN_FAVORITE, SHARE_MEDIA.SMS,SHARE_MEDIA.EMAIL,SHARE_MEDIA.MORE)
